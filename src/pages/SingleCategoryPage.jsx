...
// ─── Sorting helper — now uses centralised utility ───────────────────
...
// ═════════════════════════════════════════════════════════════════════
// SingleCategoryPage — main page component
// ═════════════════════════════════════════════════════════════════════
export default function SingleCategoryPage() {
  ...
  const categoryCreators = useMemo(() => {
    if (!category) return [];
    return mockCreators.filter((c) => c.category === category.name);
  }, [category]);

  // ── Live creators ─────────────────────────────────────────────────
  const liveCreators = useMemo(
    () => categoryCreators.filter((c) => c.isLive),
    [categoryCreators]
  );

  // ── All brands (filtered + sorted + searched) ─────────────────────
  const filteredBrands = useMemo(() => {
    let results = [...categoryCreators];
  ...
  }, [categoryCreators, platformFilter, liveOnly, searchQuery, sortBy]);
    ...
    <PageLayout fullWidth>
      ...
      <section
        ...
        style={{
          background: category.bannerImage
            ? `linear-gradient(135deg, rgba(0, 82, 212, 0.55), rgba(20, 184, 166, 0.35)), url(${category.bannerImage})`
            : category.bannerGradient,
      ...
      >
        ...
        <div className="relative z-10 container-app flex items-center justify-center h-full min-h-[160px] sm:min-h-[200px] md:min-h-[240px] py-8 sm:py-10 md:py-12">
          {/* Category name badge — only shown when no banner image (image already contains text) */}
        ...
        <div className="container-app py-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <div className="flex-1 sm:flex-none sm:w-56 md:w-64">
              <SearchBar
                placeholder={`Search ${category.name} Brands, Products...`}
                size="sm"
                value={searchQuery}
